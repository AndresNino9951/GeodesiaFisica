...
def render_header():
    ...
    # Tema académico + columna derecha (visualización) fija al hacer scroll
...
def render_3d(engine: GeodesyEngine, res: ResultadoCalculo, params: WGS84):
    ...
    fig = go.Figure(go.Surface(
        ...
        surfacecolor=V / 1e6, colorscale="Cividis",
    ...
    eje = dict(backgroundcolor="rgba(0,0,0,0)", gridcolor="rgba(212,176,76,0.22)",
               zerolinecolor="rgba(212,176,76,0.4)", color="#c9d4ea")
    fig.update_layout(
        scene=dict(
            xaxis=dict(title="X (km)", **eje),
            yaxis=dict(title="Y (km)", **eje),
            zaxis=dict(title="Z (km)", **eje),
        ),
